...
function buildTierLadder() {
  ...
  return Array.from({ length: TIER_COUNT }, (_, index) => ({
    ...
    fixedBetCost: 0n,
    useDynamicCost: true,
    costBps: 1000, // 10% of prize as direct-bet cost
  }));
}
...
function buildScaledOutcomes(): OutcomeConfig[] {
  ...
  // Tier awards (outcomes 3-11 = tiers 1-9)
  outcomes.push({ scaling: scalingRange(200, 9000, "8", "16", SCALING_QUADRATIC), tierAdvance: 1, tierResetTo: 0, consolationMultiplier: 0, awardsTier: true });   // Tier 1
  outcomes.push({ scaling: scalingRange(200, 9000, "10", "19", SCALING_QUADRATIC), tierAdvance: 1, tierResetTo: 0, consolationMultiplier: 0, awardsTier: true });  // Tier 2
  outcomes.push({ scaling: scalingRange(200, 9000, "12", "20", SCALING_QUADRATIC), tierAdvance: 1, tierResetTo: 0, consolationMultiplier: 0, awardsTier: true });  // Tier 3
  outcomes.push({ scaling: scalingRange(200, 9000, "15", "22", SCALING_QUADRATIC), tierAdvance: 1, tierResetTo: 0, consolationMultiplier: 0, awardsTier: true });  // Tier 4
  outcomes.push({ scaling: scalingRange(1000, 6000, "12", "24", SCALING_LINEAR), tierAdvance: 1, tierResetTo: 0, consolationMultiplier: 0, awardsTier: true });    // Tier 5
  outcomes.push({ scaling: scalingRange(800, 6000, "15", "27", SCALING_LINEAR), tierAdvance: 1, tierResetTo: 0, consolationMultiplier: 0, awardsTier: true });     // Tier 6
  outcomes.push({ scaling: scalingRange(1200, 6000, "24", "42", SCALING_LINEAR), tierAdvance: 1, tierResetTo: 0, consolationMultiplier: 0, awardsTier: true });     // Tier 7
  outcomes.push({ scaling: scalingRange(1200, 5000, "24", "50", SCALING_LOG), tierAdvance: 1, tierResetTo: 0, consolationMultiplier: 0, awardsTier: true });        // Tier 8
  outcomes.push({ scaling: scalingRange(50, 2000, "25", "60", SCALING_LOG), tierAdvance: 1, tierResetTo: 0, consolationMultiplier: 0, awardsTier: true });         // Tier 9
  return outcomes;
}
...
async function main() {
  ...
  // Public deployment: no whitelist, no blacklist
  ...
  console.log("Mode: PUBLIC (no whitelist, no pre-seeded referrals)");
  ...
  // PUBLIC: No whitelist, no blacklist
  // Whitelist is disabled by default, but explicitly ensure it's off
  await handler.write.setWhitelistEnabled([false], { account: deployer.account });
  console.log("Whitelist disabled (public access).");

  // PUBLIC: No referral seeding - users set their own referrers organically
  ...
  const regJ = await handler.write.registerGame([jackpot.address, jackpot.address, houseAddress, HOUSE_EDGE_BPS, REFERRAL_BPS], { account: deployer.account });
  ...
  const path = new URL("./deployments/arb-mainnet-public.json", import.meta.url);
