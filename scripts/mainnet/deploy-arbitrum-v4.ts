...
async function main() {
  ...
  tx = await jackpot.write.setConsolationProbabilities(
    [CONSOLATION_1_PROB_BPS, CONSOLATION_2_PROB_BPS],
    { account: deployer.account }
  );
  await publicClient.waitForTransactionReceipt({ hash: tx });
  console.log("   ✓ Consolation probabilities set:", CONSOLATION_1_PROB_BPS, "bps /", CONSOLATION_2_PROB_BPS, "bps");
  ...
  await publicClient.waitForTransactionReceipt({ hash: tx });
  console.log("   ✓ Table config set (jackpot:", JACKPOT_BPS, "bps =", JACKPOT_BPS / 100, "%)");
  ...
  const deployment = {
    ...
    config: {
      ...
      consolation1ProbBps: CONSOLATION_1_PROB_BPS,
      consolation2ProbBps: CONSOLATION_2_PROB_BPS,
