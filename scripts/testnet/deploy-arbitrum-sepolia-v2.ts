...
// ═══════════════════════════════════════════════════════════════════════════
// CONFIGURATION (Same as mainnet V3)
// ═══════════════════════════════════════════════════════════════════════════
...
async function main() {
  ...
  // House wallet = deployer on testnet
  const HOUSE_FALLBACK_WALLET = deployer.account.address;

  console.log("╔══════════════════════════════════════════════════════════════════╗");
  console.log("║         ARBITRUM SEPOLIA V2 DEPLOYMENT                           ║");
  console.log("║   RandomProviderV2 + JackpotV2 + RouletteV2                       ║");
  console.log("╚══════════════════════════════════════════════════════════════════╝");
  console.log("");
  ...
  // ─────────────────────────────────────────────────────────────────────────
  console.log("═══════════════════════════════════════════════════════════════════");
  console.log("1. Deploying EverValueCoin...");
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // 10. PAYMENT ONLY GAME ADAPTER (for external games)
  // ─────────────────────────────────────────────────────────────────────────
  console.log("\n═══════════════════════════════════════════════════════════════════");
  ...
  // Register adapter in payment handler
  tx = await handler.write.registerGame(
  ...
  // Fund adapter with liquidity
  tx = await token.write.transfer([adapter.address, parseEther("200")], { account: deployer.account });
  await publicClient.waitForTransactionReceipt({ hash: tx });
  console.log("   ✓ Funded adapter with 200 EVA liquidity");
  ...
  tx = await token.write.transfer([roulette.address, parseEther("500")], { account: deployer.account });
  await publicClient.waitForTransactionReceipt({ hash: tx });
  console.log("   ✓ Funded roulette with 500 EVA liquidity");

  // Approve handler for testing
  tx = await token.write.approve([handler.address, parseEther("10000")], { account: deployer.account });
  await publicClient.waitForTransactionReceipt({ hash: tx });
  console.log("   ✓ Approved handler for 10,000 EVA");
  ...
  const deploymentPath = new URL("arb-sepolia-v2.json", deploymentsDir);
  ...
  console.log("   PaymentOnlyGameAdapter:", adapter.address);
  console.log("");
  console.log("🔧 Key V2 Features:");
  console.log("   • RandomProviderV2: ~180k gas savings per VRF request");
  console.log("   • ProgressiveJackpotV2: Per-tier pots, entry-based probability");
  console.log("   • SingleRandomRouletteV2: Optional jackpot participation");
