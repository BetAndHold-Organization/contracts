# Game Integration Guide — V5 Architecture (LEGACY — SUPERSEDED)

> ⚠️ **This document predates the AuthHub / shape bases / IGameEvents work.**
> See [GAME_AUTHOR_GUIDE.md](./GAME_AUTHOR_GUIDE.md) for the current platform.
>
> What's changed since V5:
> - `BaseGame` / `VRFGameBase` / `JackpotClient` are now bundled into canonical shape bases (`PushVRFGame`, `PullVRFGame`, `OperatorGame`). Game authors inherit one shape base, not multiple mixins.
> - `SignedActionAuth` + `GameLifecycleRoles` add delegated-signing (`*For` entries) and a per-game operator allowlist.
> - `AuthHub` is the platform-wide session-key + operator registry.
> - `emergencyWithdraw` is now non-overridable on `BaseGame` and always zeros `lockedExposure`.
> - `IGameEvents` standardizes the `BetPlaced` / `BetSettled` / `BetFailed` envelope across all games.
> - `CrashGame` has been migrated to inherit `BaseGame`.
>
> Retained here for historical reference and to document the V4 → V5 migration path that may still apply to legacy game contracts still on mainnet.
