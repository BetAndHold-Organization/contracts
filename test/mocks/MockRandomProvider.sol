...
import {IRandomConsumer} from "contracts/interfaces/IRandomConsumer.sol";

contract MockRandomProvider {
    ...
    function requestRandomNumber(uint256 maxNumber) external returns (uint256 requestId) {
        ...
        nextRequestId++;
        pendingRequests[requestId] = PendingRequest({consumer: IRandomConsumer(msg.sender), maxNumber: maxNumber});
        return requestId;
    }
